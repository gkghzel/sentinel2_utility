...
# initialization
ws = "/content/sentinel2_utility"  # make sure to put the right workspace path
os.chdir(ws)
sidiZid = ws+r"/vector_data/sz/sz_limite/limiteSidiZid.shp"
tbeinya = ws+r"/vector_data/tb/tb_limite/limiteTbeinya.shp"
...
class Scene:
  ...
  def updateBands(self): # getting available bands paths (10m, 20m, 60m)
    ...
    pattern20m = '.*20m.jp2$'
    # pattern60m = '.*60m.jp2$'
    tmp = zp.ZipFile(r"./raw/"+self.fname, 'r')
